...
public class TestMain {
	...
	private static int ONE_PAIR = 100;
	private static int TWO_PAIRS = 200;
	private static int THREE_CARDS = 300;
	private static int STRAIGHT = 400;
	private static int FLUSH = 500;
	private static int FULL_HOUSE = 600;
	private static int FOUR_CARDS = 700;
	private static int STRAIGHT_FLUSH = 800;
	private static int ROYAL_FLUSH = 900;

	public static void main(String[] args) {
		int cnt = 0;

		for (String[] game : poker) {
			if (check(game)) {
				cnt++;
			}
		}
	...
	public static boolean check(String[] game) {
		int p1 = score(Arrays.copyOfRange(game, 0, 5));
		int p2 = score(Arrays.copyOfRange(game, 5, 10));
	...
	public static int score(String[] cards) {
		...
		HashSet<Character> suits = new HashSet<Character>();
		HashMap<Integer, Integer> ranks = new HashMap<Integer, Integer>();

		for (String card : cards) {
			suits.add(card.charAt(0));
		...
		Integer[] keys = ranks.keySet().toArray(new Integer[0]);
		Arrays.sort(keys);

		if (ranks.size() == 2) {
			switch (ranks.get(keys[0])) {
			case 1:
				return FOUR_CARDS + keys[1];
			case 4:
				return FOUR_CARDS + keys[0];
			case 2:
				return FULL_HOUSE + keys[1];
			case 3:
				return FULL_HOUSE + keys[0];
			}
		} else if (ranks.size() == 3) {
			int max = -1;

			for (int key : keys) {
				switch (ranks.get(key)) {
				case 3:
					return THREE_CARDS + max;
				case 2:
					max = Math.max(key, max);
				}
			}

			return TWO_PAIRS + max;
			...
			if (isFlush && isStraight) {
				return keys[0] == 10 ? ROYAL_FLUSH : STRAIGHT_FLUSH + keys[4];
			} else if (isFlush) {
				return FLUSH + keys[4];
			} else if (isStraight) {
				return STRAIGHT + keys[4];
			} else {
				return HIGH_CARD + keys[4];
			}
		}
	...
	private static String[][] poker = { { "8C", "TS", "KC", "9H", "4S", "7D", "2S", "5D", "3S", "AC" },
